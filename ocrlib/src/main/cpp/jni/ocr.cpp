...
#define TAG "ocr_"
    ...
    OCR::OCR(AAssetManager *manager, char *p) {
        std::string path(p);
        ...
        for (int16_t i = 0; i < h; ++i) {
            for (int16_t j = 0; j < w; ++j) {
                ...
                if (point[index]) {
                    ...
                    LOGD(TAG, "queue size=%ld score=%.3f value=%d", queue.size(), score_sum, value);
    ...
    int pse_decode(ncnn::Mat &features,
        ...
        ncnn::Mat kernel(w, h, features.c, (size_t) 1);// 每一层的识别数据
        ncnn::Mat scores(w, h, (size_t) 2);
        int16_t *scores_data = (int16_t *) scores.data;
        LOGD(TAG, "parse score");
        double st = ncnn::get_current_time();

        float *src[features.c];
        int8_t *kernel_data[kernel.c];
        for (int c = 0; c < features.c; ++c) {
            kernel_data[c] = (int8_t *) kernel.data + kernel.cstep * c;
            src[c] = (float *) features.data + features.cstep * c;
        }
        omp_set_num_threads(4);
#pragma omp parallel for
        for (int i = 0; i < size; i++) {
            // 已知features.c为6，所以这里直接写6条，可用循环处理
            kernel_data[0][i] = (int8_t) (src[0][i] >= _thresh ? 1 : 0);
            kernel_data[1][i] = (int8_t) (src[1][i] >= _thresh ? 1 : 0);
            kernel_data[2][i] = (int8_t) (src[2][i] >= _thresh ? 1 : 0);
            kernel_data[3][i] = (int8_t) (src[3][i] >= _thresh ? 1 : 0);
            kernel_data[4][i] = (int8_t) (src[4][i] >= _thresh ? 1 : 0);
            kernel_data[5][i] = (int8_t) (src[5][i] >= _thresh ? 1 : 0);
            // 将分数放大到10000倍存储，以节省空间
            scores_data[i] = (int16_t) (src[5][i] * 10000);
        }
        LOGI(TAG, "kernel time=%.3lf", ncnn::get_current_time() - st);

        ncnn::Mat mask(w, h, (size_t) 1);
        std::map<int, std::queue<Point2i>> map;

        const int count = mark((int8_t *) kernel.data, scores, mask, map);
    ...
    std::vector<std::string> OCR::detect(ncnn::Mat img) {
        LOGD(TAG, "detect");
...
//            int min_size = w > h ? h : w;
            ...
            LOGI(TAG, "get rect rgb time=%.2lf", ncnn::get_current_time() - st);
/*
            //分类 计算文本的方向 c=3，宽高固定
...
//            saveBitmap(crnn_in, "crnn_in", i);
            LOGI(TAG, "get rect gray time=%.2lf", ncnn::get_current_time() - st);
            st = ncnn::get_current_time();
